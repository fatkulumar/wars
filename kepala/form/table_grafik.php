...
<div class="modal fade" id="modalTK" tabindex="-1" role="dialog" aria-labelledby="modalPembayaranLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="proses/pdf_tk.php?print_tk=<?= $row_tk['gelombang_ke'] ?>" target="_blank"><i class="fa fa-print"></i></a></td>
                        </tr>
                        <?php endwhile ?>
                    </tbody>
                </table>
        ...
        <div class="modal-footer">
            ...
            <a href="proses/pdf_tk.php?print_tk" target="_blank" type="button" class="btn btn-primary"><i class="fa fa-print"></i></a>
        </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="modalKB" tabindex="-1" role="dialog" aria-labelledby="modalPembayaranLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
            ...
            <div class="table table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php 
                            $no = 1;
                            $sql_gel_kb = mysqli_query($koneksi, "SELECT `gelombang_ke`, count('gelombang_ke') AS jml_gel_kb FROM `tb_pembayaran` WHERE `jenis_pendidikan` = 'kb' GROUP BY `gelombang_ke`");
                            while($row_kb = mysqli_fetch_array($sql_gel_kb)): 
                        ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><?= $row_kb["gelombang_ke"] ?></td>
                            <td><?= $row_kb["jml_gel_kb"] ?></td>
                            <td><a target="a_blank" href="proses/pdf_kb.php?print_kb=<?= $row_kb['gelombang_ke'] ?>"><i class="fa fa-print"></i></a></td>
                        </tr>
                        <?php endwhile ?>
                    </tbody>
                </table>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <a target="_blank" href="proses/pdf_kb.php?print_kb" type="button" class="btn btn-primary"><i class="fa fa-print"></i></a>
        </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="modalTPA" tabindex="-1" role="dialog" aria-labelledby="modalPembayaranLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
        ...
        <div class="table table-responsive">
                <table>
                    ...
                    <tbody>
                        <?php 
                            $no = 1;
                            $sql_gel_tpa = mysqli_query($koneksi, "SELECT `gelombang_ke`, count('gelombang_ke') AS jml_gel_tpa FROM `tb_pembayaran` WHERE `jenis_pendidikan` = 'tpa' GROUP BY `gelombang_ke`");
                            while($row_tpa = mysqli_fetch_array($sql_gel_tpa)): 
                        ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><?= $row_tpa["gelombang_ke"] ?></td>
                            <td><?= $row_tpa["jml_gel_tpa"] ?></td>
                            <td><a target="a_blank" href="proses/pdf_tpa.php?print_tpa=<?= $row_tpa['gelombang_ke'] ?>"><i class="fa fa-print"></i></a></td>
                        </tr>
                        <?php endwhile ?>
                    </tbody>
                </table>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <a href="proses/pdf_kb.php?print_tpa" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i></a>
        </div>
        </div>
    </div>
</div>