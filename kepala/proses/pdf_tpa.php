...
if($_GET["print_tk"] == null){
    $sql_tk = mysqli_query($koneksi, "SELECT `id_user_pembayaran`, `nama_pembayaran`, `gelombang_ke`, `biaya_gelombang`, `status_pembayaran`, `nama_user`, `nama_wali`, `nama_ibu`, `nama_anak` FROM tb_pembayaran p JOIN tb_user u JOIN tb_wali w ON w.id_user_wali = u.id_user JOIN tb_ibu i ON i.id_wali_ibu = w.id_wali JOIN tb_anak a ON a.id_wali_anak = w.id_wali WHERE p.jenis_pendidikan = 'tpa' GROUP BY id_user_pembayaran");
}else{
    $sql_tk = mysqli_query($koneksi, "SELECT `id_user_pembayaran`, `nama_pembayaran`, `gelombang_ke`, `biaya_gelombang`, `status_pembayaran`, `nama_user`, `nama_wali`, `nama_ibu`, `nama_anak` FROM tb_pembayaran p JOIN tb_user u JOIN tb_wali w ON w.id_user_wali = u.id_user JOIN tb_ibu i ON i.id_wali_ibu = w.id_wali JOIN tb_anak a ON a.id_wali_anak = w.id_wali WHERE p.gelombang_ke = '$id' AND p.jenis_pendidikan = 'tpa' GROUP BY id_user_pembayaran");
}
...
<div style="text-align: center; position: relative;">
    <strong>Laporan Pendaftaran TPA</strong> <br>
